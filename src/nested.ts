...
export function findQuestion(
    ...
    const target = questions.find((q: Question): boolean => q.id === id);
...
export function sumPoints(questions: Question[]): number {
    return questions.reduce((curr: number, q: Question) => curr + q.points, 0);
}
...
export function sumPublishedPoints(questions: Question[]): number {
    const pubbed = getPublishedQuestions(questions);
    return sumPoints(pubbed);
}
...
export function addNewQuestion(
    ...
    return [...copy, makeBlankQuestion(id, name, type)];
}
...
export function duplicateQuestionInArray(
    ...
    const dupQ = duplicateQuestion(newId, questions[ind]);
    const dup = questions.map(
        (q: Question): Question => ({ ...q, options: [...q.options] })
    );
    dup.splice(ind + 1, 0, dupQ);
    return dup;
}
